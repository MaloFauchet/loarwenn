...
class Caroussel {
    ...
    createPagination(){
        let pagination = this.createDivWithClass('caroussel__pagination')
        let buttons = []
        this.root.appendChild(pagination)
        for (let index = 0; index < (this.items.length -2 * this.offset); index= index+this.options.slidesToScroll) {
            let button = this.createDivWithClass('caroussel__pagination__button');
        ...
        this.onMove(index => {
            ...
            if (activeButton) {
                buttons.forEach(
                    button => button.classList.remove('caroussel__pagination__button--active')
                )
                activeButton.classList.add('caroussel__pagination__button--active')
            }
        }
